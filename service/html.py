...
class HTML:
    """
    Class to generate the documentation in HTML format using Bootstrap.
    """
    ...
    def gerar_html(self) -> str:
        ...
        def gerar_html_cabecalho(self) -> str:
            """
            Gera o cabeçalho do HTML, com o nome do modelo e a data do relatório.
            :return: str
            """
            retorno = f'''
            <div class="row pt-4">
        ...
        def gerar_html_resumo_modelo_relacionamentos(self) -> str:
            ...
            for r in self.model.relationships:
                ...
                if r.is_active:
                    retorno += f'<tr><td>{origin}</td><td>{r.origin_cardinality}</td><td>{direction}</td><td>{r.target_cardinality}</td><td>{target}</td></tr>'
                else:
                    retorno += f'<tr><td><em>(Desativado) {origin}</em></td><td><em>{r.origin_cardinality}</em></td><td>{direction}</td><td><em>{r.target_cardinality}</em></td><td><em>{target}</em></td></tr>'
            retorno += f'</tbody></table></div></div>'
            return retorno
                ...
                <div class="accordion-item">
                        ...
                        <div id="collapse{count}" class="accordion-collapse collapse {"" if count > 1 else "show"}">
                            ...
                            <ul>
                                ...
                                <li><strong>Tipo:</strong> {t.table_type}</li>
                                <li><strong>Modo de importação:</strong> {t.import_mode}</li>
                            </ul>
                            <h5>Power Query</h5>
...
{query}
                ...
                if columns:
                    ...
                    '''
                    for c in columns:
                        retorno += f'<tr><td>{c.name}</td><td>{c.data_type}</td></tr>'
                    retorno += f'</tbody></table></div>'
                if measures:
                    retorno += '''
                    <div class="col-md-4">
                    <table class="table table-bordered table-hover table-striped small">
                    ...
                    '''
                    for m in measures:
                        retorno += f'<tr><td><a href="#{m.table_item_id}">{m.name}</a></td></tr>'
                    retorno += f'</tbody></table></div>'
                if calculated_columns:
                    ...
                    for cc in calculated_columns:
                        retorno += f'<tr><td><a href="#{cc.table_item_id}">{cc.name}</a></td><td>{cc.data_type}</td></tr>'
                    retorno += f'</tbody></table></div>'
                retorno += '</div>'
